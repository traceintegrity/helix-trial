...
def _build_program_landing_html(output_root: Path, latest_legacy_dashboard_name: str | None) -> tuple[str, str, str]:
    ...
    corpus_summary = _load_latest_csv_row(output_root / "corpus", "*_trace_corpus_summary.csv")
    archive_summary = _load_latest_csv_row(output_root / "archive", "*_trace_archive_summary.csv")
    scorecard_summary = _load_latest_csv_row(output_root / "scorecard", "*_trace_scorecard_summary.csv")
    ...
    corpus_source_count = int(corpus_summary.get("source_count", 0)) if corpus_summary is not None else 0
    source_family_count = int(corpus_summary.get("source_family_count", 0)) if corpus_summary is not None else 0
    gateway_batches = int(gateway_status.get("ingested_batches", 0)) if gateway_status is not None else 0
    gateway_degraded = bool(gateway_status.get("degraded_mode", False)) if gateway_status is not None else False
    sidecar_degraded = bool(sidecar_status.get("degraded_mode", False)) if sidecar_status is not None else False
    recommended_cap = "n/a"
    if archive_summary is not None:
        recommended_cap = str(archive_summary.get("recommended_max_active_templates", "n/a"))
    elif corpus_summary is not None:
        recommended_cap = str(corpus_summary.get("recommended_max_active_templates", "n/a"))
    elif trace_summary is not None:
        recommended_cap = str(trace_summary.get("recommended_max_active_templates", "n/a"))
    tuning_uplift = (
        float(archive_summary.get("avg_recommended_gain_delta_vs_baseline", 0.0))
        if archive_summary is not None
        else float(corpus_summary.get("avg_recommended_gain_delta_vs_baseline", 0.0))
    ...
    replay_passes = (
        int(archive_summary.get("stream_replay_passes", 0))
        if archive_summary is not None
        else int(corpus_summary.get("stream_replay_passes", 0))
        if corpus_summary is not None
        else int(trace_summary.get("stream_replay_passes", 0)) if trace_summary is not None else 0
    )
    archive_gain = int(archive_summary.get("archive_bundle_gain_vs_raw_zlib", 0)) if archive_summary is not None else None
    replay_verified = (
        bool(int(corpus_summary.get("canonical_roundtrip_verified", 0)))
        if corpus_summary is not None
        else bool(int(trace_summary.get("canonical_roundtrip_verified", 0))) if trace_summary is not None else None
    )
    reproducibility_verified = (
        bool(int(corpus_summary.get("reproducibility_verified", 0)))
        if corpus_summary is not None
        else bool(int(trace_summary.get("reproducibility_verified", 0))) if trace_summary is not None else None
    )
    positive_archive_gain_source_count = (
        int(archive_summary.get("positive_archive_gain_source_count", 0))
        if archive_summary is not None
        else None
    )
    negative_archive_gain_source_count = (
        int(archive_summary.get("negative_archive_gain_source_count", 0))
        if archive_summary is not None
        else int(corpus_summary.get("negative_source_gzip_gain_count", 0)) if corpus_summary is not None else None
    )
    ...
    beta_gate = evaluate_beta_gate(
        public_context={
            ...
            "positive_archive_gain_source_count": positive_archive_gain_source_count or 0,
            "negative_archive_gain_source_count": negative_archive_gain_source_count or 0,
            "negative_source_families": [],
            "replay_verified": int(replay_verified is True),
            "reproducibility_verified": int(reproducibility_verified is True),
        },
        partner_context=None,
        worker_context=None,
        output_root=output_root,
    )
    release_state = str(beta_gate["release_state"])

    where_we_are = _where_we_are_text(legacy_summary, trace_summary, corpus_summary, gateway_status, archive_summary)
    honest_review = _honest_review_text(legacy_summary, trace_summary, corpus_summary, gateway_status, archive_summary)
    implications = _implications_text(trace_summary, corpus_summary, gateway_status, archive_summary)
    real_world = (
        "In real-world terms, this is now closest to a specialized semantic packing layer beside an OpenTelemetry/OpenInference ingest pipeline. "
        "It is not a full observability platform, not a frontier model, and not a general-purpose compressor."
    )
    frontier_steps = (
        "1. Run on real partner AI trace exports.\n"
        "2. Tune template retention on production-shaped workloads.\n"
        "3. Prove savings against existing storage and transport paths.\n"
        "4. Add deployment-facing ingestion hooks instead of only offline runners."
    )
    board_watchouts = (
        "1. We still need more real AI trace exports than the current small corpus.\n"
        "2. Retention tuning is present, but it is still validated on local workloads rather than partner traffic.\n"
        "3. The legacy sequence-policy branch remains useful R&D, not the shipping product.\n"
        "4. Product credibility now depends more on integrations and partner data than on more synthetic experiments."
    )
    legacy_eval_ratio = float(legacy_summary.get("eval_net_compression_ratio", 0.0)) if legacy_summary is not None else None
    trace_gain = int(trace_summary.get("stream_pack_gain_vs_windowed_raw_zlib", 0)) if trace_summary is not None else None
    board_letter = build_board_letter(
        ...
        partner_context=None,
        launch_gate=str(beta_gate["launch_gate"]),
        next_commercial_step=str(beta_gate["next_commercial_step"]),
    )

    artifact_cards = "\n".join(
        [
            _render_artifact_row("Latest Trace Report", "latest_trace_benchmark.html"),
            _render_artifact_row("Latest Corpus Report", "latest_trace_corpus.html"),
            _render_artifact_row("Latest Archive Report", "latest_trace_archive.html" if archive_summary is not None else "archive adapter not run yet"),
            _render_artifact_row("Latest Workflow Scorecard", "latest_trace_scorecard.html" if scorecard_summary is not None else "workflow scorecard not run yet"),
            _render_artifact_row("Pilot Sidecar Status", "sidecar/status.json" if sidecar_status is not None else "pilot sidecar not run yet"),
            _render_artifact_row("Version History", "version_history.html"),
            _render_artifact_row("Latest Board Letter", "latest_board_letter.html"),
            _render_artifact_row("Release Gate", "latest_beta_gate.html" if (output_root / "dashboard" / "latest_beta_gate.html").exists() else "beta gate not built yet"),
            _render_artifact_row("Public Release Brief", "latest_public_release_brief.html" if (output_root / "dashboard" / "latest_public_release_brief.html").exists() else "public release brief not built yet"),
            ...
            _render_artifact_row("Gateway Status", "gateway/status.json" if gateway_status is not None else "gateway not started yet"),
        ]
    )
    status_cards = "\n".join(
        [
            _render_metric_card("Release State", release_state, "Current honest release posture for the product path."),
            _render_metric_card(
                "Release Readiness",
                "passing" if int(beta_gate["public_beta_ready"]) else "blocked",
                "Whether the current product clears the public-beta gate instead of only the design-partner beta bar.",
            ),
            _render_metric_card(
                "Latest Trace Gain",
                _format_signed_int(int(trace_summary.get("stream_pack_gain_vs_windowed_raw_zlib", 0))) if trace_summary is not None else "n/a",
                "Streaming byte gain versus compressing each raw window independently.",
            ),
            _render_metric_card(
                "Archive Bundle Gain",
                _format_signed_int(archive_gain) if archive_gain is not None else "n/a",
                "Gain from the archive bundle versus raw JSON plus commodity compression.",
            ),
            _render_metric_card(
                "Pilot-Ready Workflows",
                str(int(scorecard_summary.get("pilot_ready_workflow_count", 0))) if scorecard_summary is not None else "0",
                "How many workflow families currently clear the scorecard gate for pilot consideration.",
            ),
            _render_metric_card(
                "Pilot Sidecar Batches",
                str(int(sidecar_status.get("gateway", {}).get("ingested_batches", 0))) if sidecar_status is not None else "0",
                "How many OTLP batches have gone through the unified pilot sidecar path.",
            ),
            _render_metric_card(
                "Gateway Mode",
                "degraded" if gateway_degraded else ("healthy" if gateway_status is not None else "n/a"),
                "Whether the OTLP gateway had to accept raw traffic while reporting a downstream refresh problem.",
            ),
            _render_metric_card(
                "Sidecar Mode",
                "degraded" if sidecar_degraded else ("healthy" if sidecar_status is not None else "n/a"),
                "Whether the sidecar preserved ingest while archive or scorecard processing was failing.",
            ),
            _render_metric_card(
                "Recommended Active Cap",
                recommended_cap,
                "Current best active-template cap from the latest retention tuning pass.",
            ),
            _render_metric_card(
                "Avg Tuning Uplift",
                f"{tuning_uplift:+.2f}" if trace_summary is not None or corpus_summary is not None else "n/a",
                "Estimated stream-gain improvement versus the current baseline cap.",
            ),
            _render_metric_card(
                "Corpus Files",
                str(corpus_source_count) if corpus_summary is not None else "0",
                "How many exported trace files are currently covered by the latest corpus run.",
            ),
            _render_metric_card(
                "Corpus Families",
                str(source_family_count) if corpus_summary is not None else "0",
                "How many distinct workload families are represented in the latest public corpus run.",
            ),
            _render_metric_card(
                "Replay Integrity",
                "yes" if replay_verified else ("no" if replay_verified is False else "n/a"),
                "Whether the latest trace pack still decodes back to the exact canonical trace payload.",
            ),
            _render_metric_card(
                "Reproducible",
                "yes" if reproducibility_verified else ("no" if reproducibility_verified is False else "n/a"),
                "Whether the latest benchmark reruns to the same semantic payload deterministically.",
            ),
            _render_metric_card(
                "Archive-Positive Families",
                str(positive_archive_gain_source_count) if positive_archive_gain_source_count is not None else "n/a",
                "How many latest archive source families beat the raw source plus gzip baseline.",
            ),
            _render_metric_card(
                "Negative Cases",
                str(negative_archive_gain_source_count) if negative_archive_gain_source_count is not None else "n/a",
                "How many source families still lose and therefore need to stay explicitly disclosed in public materials.",
            ),
            _render_metric_card(
                "Replay Passes",
                str(replay_passes) if replay_passes else "0",
                "How aggressively the stream is being replayed to test longer-lived retention economics.",
            ),
            _render_metric_card(
                "Legacy Held-Out Net",
                _format_float(float(legacy_summary.get("eval_net_compression_ratio", 0.0))) if legacy_summary is not None else "n/a",
                "The old sequence-policy research track. Below 1.00 would mean held-out net-positive compression there.",
            ),
            _render_metric_card(
                "Gateway Batches",
                str(gateway_batches) if gateway_status is not None else "0",
                "How many OTLP batches the local gateway has accepted so far.",
            ),
        ]
    )
    prototype_cards = "\n".join(
        [
            _render_program_prototype_card(
                "Gateway Sidecar Prototype",
                "Live locally" if gateway_batches > 0 else "Ready locally",
                (
                    f"Attach the OTLP gateway to an agent service, ingest live traces, and refresh the corpus report automatically. "
                    f"The current recommended active-template cap is {recommended_cap}."
                ),
                "Use this to show a design partner how semantic packing would sit beside their existing OpenTelemetry pipeline.",
            ),
            _render_program_prototype_card(
                "Archive Backfill Prototype",
                "Running on archive bundles" if archive_summary is not None else ("Running on export sets" if corpus_source_count > 0 else "Awaiting corpus run"),
                (
                    f"Process exported trace directories through the archive adapter to benchmark storage-path savings. "
                    f"The latest bundle gain is {archive_gain:+d} bytes versus raw zlib."
                    if archive_gain is not None
                    else f"Process exported trace directories through the corpus runner to validate repeated gains across {corpus_source_count} file"
                    f"{'' if corpus_source_count == 1 else 's'} with replay-pass stress testing."
                ),
                "Use this for backfill and cost analysis before attempting inline deployment.",
            ),
            _render_program_prototype_card(
                "Replay And Audit Prototype",
                "Ready",
                "Generate semantic packs plus manifests, then unpack them for replay and investigation. This makes the product legible as storage-and-replay infrastructure rather than just a benchmark.",
                "Use this to demonstrate operational value beyond pure compression ratios.",
            ),
            _render_program_prototype_card(
                "Pilot Sidecar Prototype",
                "Live locally" if sidecar_status is not None else "Ready locally",
                (
                    f"Expose OTLP ingest, archive refresh, scorecard refresh, and pilot status from one local sidecar. "
                    f"The current sidecar has processed {int(sidecar_status.get('gateway', {}).get('ingested_batches', 0)) if sidecar_status is not None else 0} batch"
                    f"{'' if sidecar_status is not None and int(sidecar_status.get('gateway', {}).get('ingested_batches', 0)) == 1 else 'es'} and is currently "
                    f"{'degraded' if sidecar_degraded else 'healthy'}."
                ),
                "Use this as the nearest real pilot shape before touching a partner environment.",
            ),
        ]
    )
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta http-equiv="refresh" content="10">
  <title>Helix - Program Dashboard</title>
  <style>
    :root {{
      --bg: #f4efe6;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero {{ padding: 28px; background: linear-gradient(135deg, #15302b, #3d7267); color: #f8f4ee; position: relative; overflow: hidden; }}
    .hero::after {{ content: ""; position: absolute; inset: auto -44px -76px auto; width: 240px; height: 240px; background: radial-gradient(circle, rgba(248, 226, 195, 0.28), transparent 70%); }}
    h1, h2, h3 {{ font-family: "Iowan Old Style", Georgia, serif; margin: 0; }}
    .hero p {{ margin: 14px 0 0; max-width: 920px; color: rgba(248, 244, 238, 0.88); line-height: 1.6; }}
    ...
    .section {{ margin-top: 20px; }}
    .section-header {{ display: flex; flex-wrap: wrap; justify-content: space-between; gap: 12px; margin-bottom: 12px; align-items: end; }}
    .section-header p {{ margin: 0; color: var(--muted); line-height: 1.55; max-width: 780px; }}
    ...
    .metric-card span {{ display: block; font-size: 0.76rem; text-transform: uppercase; letter-spacing: 0.08em; color: var(--muted); }}
    .metric-card strong {{ display: block; margin-top: 8px; font-size: 1.9rem; color: var(--ink); }}
    .artifact-list {{ display: grid; gap: 10px; }}
    code {{ font-family: "Consolas", monospace; background: rgba(22, 48, 43, 0.06); padding: 0.12rem 0.35rem; border-radius: 0.3rem; word-break: break-word; }}
  ...
  <main class="page">
    <section class="hero">
      ...
      <h1>Program Dashboard</h1>
          <p>This is the global landing page for the whole project. It now acts as dashboard, board brief, release-readiness view, and product prototype summary in one place so the current path stays legible as the system changes.</p>
      <div class="hero-actions">
        <a class="pill" href="latest_trace_benchmark.html">Latest Trace Report</a>
        <a class="pill" href="latest_trace_corpus.html">Latest Corpus Report</a>
        <a class="pill" href="latest_board_letter.html">Board Letter</a>
        <a class="pill" href="version_history.html">Version History</a>
        <span class="pill">{escape(release_state)}</span>
        <span class="pill">Recommended Cap {escape(recommended_cap)}</span>
      </div>
    </section>
    ...
    <section class="section">
      <div class="section-header">
        <div>
          <h2>Current State</h2>
          <p>These cards summarize the current release posture across the whole program.</p>
        </div>
      </div>
      <div class="grid">
        {status_cards}
    ...
    <section class="section">
      <div class="section-header">
        <div>
          ...
          <p>This strip is the public-release gate. It tracks the realities that matter for a self-hosted launch: corpus diversity, replay integrity, archive-positive families, disclosed negative cases, and deterministic reproducibility.</p>
        </div>
      </div>
      <div class="grid">
        <article class="card"><h3>Readiness Read</h3><p>The current posture is <strong>{escape(release_state)}</strong>. To earn a real public release, the archive path must keep showing repeated savings on multiple workload families while replay and reproducibility stay exact.</p></article>
        <article class="card"><h3>Positive Evidence</h3><p>The latest corpus covers {corpus_source_count} file{'' if corpus_source_count == 1 else 's'} across {source_family_count} workload famil{'y' if source_family_count == 1 else 'ies'}. Archive-positive families: {escape(str(positive_archive_gain_source_count)) if positive_archive_gain_source_count is not None else 'n/a'}. Replay verified: {escape('yes' if replay_verified else 'no' if replay_verified is False else 'n/a')}.</p></article>
        <article class="card"><h3>Negative Disclosure</h3><p>Negative cases are part of the product truth. The latest run still shows {escape(str(negative_archive_gain_source_count)) if negative_archive_gain_source_count is not None else 'n/a'} family{'ies' if negative_archive_gain_source_count not in {None, 1} else 'y'} where the current archive path is not yet net-positive.</p></article>
        <article class="card"><h3>Reproducibility</h3><p>Benchmark reproducibility is {escape('passing' if reproducibility_verified else 'failing' if reproducibility_verified is False else 'not published')} and the current recommended active-template cap is {escape(recommended_cap)}.</p></article>
      </div>
    </section>
    ...
    <section class="section">
      <div class="section-header">
        <div>
          <h2>Where We Are</h2>
          <p>A simple, honest read of the product and research state right now.</p>
        </div>
      </div>
      <div class="grid">
        <article class="card"><h3>Program State</h3><p>{escape(where_we_are)}</p></article>
        <article class="card"><h3>Honest Review</h3><p>{escape(honest_review)}</p></article>
        <article class="card"><h3>Implications</h3><p>{escape(implications)}</p></article>
        <article class="card"><h3>Real-World Comparison</h3><p>{escape(real_world)}</p></article>
      </div>
    </section>
    ...
    <section class="section">
      <div class="section-header">
        <div>
          ...
          <p>The board letter is now generated automatically after each version refresh so the project stays explainable in plain English.</p>
        </div>
      </div>
      <div class="grid">
        <article class="card"><h3>{escape(board_letter.title)}</h3><p>{escape(board_letter.paragraph_one)}</p><p>{escape(board_letter.paragraph_two)}</p><p><strong>Launch Gate:</strong> {escape(board_letter.launch_gate)}</p><p><strong>Next Commercial Step:</strong> {escape(board_letter.next_commercial_step)}</p></article>
        <article class="card"><h3>Board Letter Artifact</h3><p>Use the standalone board letter page for meeting prep or async review. It stays in sync with the latest landing page state.</p><p><code>outputs/dashboard/latest_board_letter.html</code></p><p><code>outputs/dashboard/latest_board_letter.md</code></p></article>
      </div>
    </section>
    ...
    <section class="section">
      <div class="section-header">
        <div>
          ...
          <p>These cards explain how the product can be piloted today using the code already in the repo.</p>
        </div>
      </div>
      <div class="grid">
        {prototype_cards}
    ...
    <section class="section">
      <div class="section-header">
        <div>
          <h2>Frontier Gap</h2>
          <p>What still separates this from frontier-level tools.</p>
        </div>
      </div>
      <div class="grid">
        <article class="card"><h3>What We Need Next</h3><p>{escape(frontier_steps)}</p></article>
        <article class="card"><h3>Board Watchouts</h3><p>{escape(board_watchouts)}</p></article>
